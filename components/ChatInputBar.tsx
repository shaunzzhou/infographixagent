...
export const ChatInputBar: React.FC<ChatInputBarProps> = ({
  ...
  return (
    <div
      className={`w-full rounded-2xl border ${isDragOver ? 'border-blue-500' : 'border-gray-700 dark:border-slate-700'} bg-[#0f172a] dark:bg-slate-900 shadow-lg shadow-slate-900/40 overflow-hidden transition-colors`}
      onDrop={onDrop}
    ...
    >
      ...
      <div className="p-0 space-y-3">
        {file && (
          <div className="px-4 pt-3">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-800 border border-slate-700 text-sm text-gray-100">
        ...
        <div>
          <div className="bg-slate-950 border border-slate-800 px-3 py-2">
            <textarea
              value={text}
              onChange={(e) => onTextChange(e.target.value)}
              placeholder={placeholder}
              className="w-full bg-transparent border-none outline-none resize-none text-sm text-slate-50 placeholder:text-slate-500 min-h-[140px]"
              rows={4}
            />
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 px-3 py-2 border-t border-slate-800 bg-slate-900">
        <div className="flex items-center gap-3">
          <button
            onClick={() => fileInputRef.current?.click()}
            className="w-9 h-9 rounded-full bg-slate-800 text-slate-100 flex items-center justify-center hover:bg-slate-700 transition-colors border border-slate-700"
            title="Upload file"
          >
            <Plus className="w-5 h-5" />
          </button>
          <div className="flex items-center gap-2 text-slate-100 h-11">
            <span className="text-sm font-semibold">Mode:</span>
            <select
              value={modePreference}
              onChange={(e) => onModeChange(e.target.value as 'AUTO' | AnalysisMode)}
              className="h-9 px-3 rounded-lg text-sm font-medium border bg-slate-950 border-slate-800 text-slate-100 focus:outline-none"
            >
